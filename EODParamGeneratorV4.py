...
def ddlCreator(mRecType, paramsElements, prefaceHdr, s3Details, maskFlag, cobolFlag, argTID):                # C20210108
    global createDB
    elemFields = list()
    subFileCount = 1
    for each in prefaceHdr+paramsElements:
        if type(each) is dict:
            ...
            argSubTID = argTID + '_' + str(subFileCount)
            ...
            ddlCreator(mRecType, each[:-1], prefaceHdr + [indxElement], s3SubDetails, maskFlag,
                       cobolFlag, argSubTID)                                                                            # C20210108
            subFileCount += 1
        else:
    ...
    for indx, each in enumerate(elemFields, 1):
        ...
        elif each['DataType'] in ['text', 'null']:
        ...
        if argTID in finalRes:
            finalRes[argTID].append(each["Element"])
        else:
            finalRes[argTID] = [each["Element"]]
    if '/current/' not in s3Details[1]:
        raise Exception("ddlFor value is incorrect in params. change it to current ")
    finalRes[f'{argTID}_CurrentFolder'] = s3Details[1]

    # finalRes[f'{argTID}AthenaTable'] = f'{dbName}.{tblName}'
...
def processingParams(ppath, infile, cobolfile=False, masked=False):                                          # C20210108
    ...
    outWriter = open(ddlDirPath + f'{params["Company"]}_{parmFileName}_SchemaDetails.adhoc', 'w+')
    finalRes["EODFolder"] = params.get("EODFolder")
    # if ddlFor in parquetDDLs:
    #     finalRes["EODFolder"] = f'{params.get("EODFolder").strip()}-parquet'
    # else:
    #     finalRes["EODFolder"] = params.get("EODFolder")
    ...
    for recType in params["RecordTypes"]:
        ...
        ddlCreator(recType.strip(), params['RecordTypes'][recType]['Elements'],
                   prefaceHeader, dataBucketDetails, masked, cobolfile, recType.strip())                     # C20210108
        # finalRes[f'{recType.strip()}Folder'] = params["RecordTypes"][recType]["PreviousFolder"].rsplit('/',1)[1]
    json.dump(finalRes, outWriter, indent=2)
    ...
    for rootDir, folders, fileNames in os.walk(paramDirPath):
        ...
        for eachFile in fileNames:
            ...
            try:
                ...
                finalRes = {}
                processingParams(paramDirPath, eachFile, cobolfile=listFileInfo[0] == listParamsCheck[1])    # C20210108
